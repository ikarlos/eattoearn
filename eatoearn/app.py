...
#
payout_collection = db.get_collection("payouts")
...
async def search_food(query: str = None, admin_uid: str = None):
...
# Earnings Total
@app.get("/api/earnings_total/")
async def get_earnings_total(admin_uid: str, user_uid: str):
    if await is_admin(user_uid=admin_uid):
        current_income = await income_collection.find_one({"user_uid": user_uid})
        recent_payout = (
            await payout_collection.find({"user_uid": user_uid})
            .sort("_id", DESCENDING)
            .limit(1)
            .to_list(1)
        )
        ...
        total_amount = 0.0
        if (
            current_income.get("active_income") is not None
            and current_income.get("active_income") > 0
        ):
            total_amount += current_income.get("active_income")

        if (
            current_income.get("passive_income") is not None
            and current_income.get("passive_income") > 0
        ):
            total_amount += current_income.get("passive_income")

        return {
            "user_uid": user_uid,
            "total_amount": total_amount,
            "recent_payout_amount": (
                recent_payout[0]["payout_amount"] if len(recent_payout) == 1 else None
            ),
        }
...
@app.post("/api/earnings_payed_all/")
async def earnings_payed_all(earning_admin: AdminWithId):
    if await is_admin(user_uid=earning_admin.admin_uid):
        current_income = await income_collection.find_one(
            {"user_uid": earning_admin.id}
        )
        total_amount = 0.0
        #
        if (
        ...
        await income_collection.update_one(
            {"user_uid": earning_admin.id},
            {"$set": {"active_income": 0.0, "passive_income": 0.0}},
        )
        # Update documents where user_uid matches and set the "payed" field to True
        await earning_collection.update_many(
            {"user_uid": earning_admin.id}, {"$set": {"payed": True}}
        )
        # Set recent payout
        await payout_collection.insert_one(
            {"user_uid": earning_admin.id, "payout_amount": total_amount}
        )

        # Ret...
        return {
            "status": CODE_SUCCESS,
            "message": "all earnings marked as payed successfully",
        }

    raise HTTPException(status_code=404, detail=f"ERROR_NO_ADMIN")


@app.post("/api/earnings_delete_all/")
async def earnings_delete_all(earning_admin: AdminWithId):
    if await is_admin(user_uid=earning_admin.admin_uid):
        # Update documents where user_uid matches and set the "payed" field to True
        await earning_collection.delete_many(
            {"user_uid": earning_admin.id, "payed": True}
        )
